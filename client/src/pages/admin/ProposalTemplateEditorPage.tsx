...
import { ProposalContentEditor } from '../../components/crm/ProposalContentEditor';
...
type RenderMode = 'document' | 'canvas';
...
export const ProposalTemplateEditorPage: React.FC = () => {
  ...
  const [docAiError, setDocAiError] = useState('');
  const [docAiProvider, setDocAiProvider] = useState('');
  ...
  const [canvasAiInstruction, setCanvasAiInstruction] = useState('');
  const [canvasAiLoading, setCanvasAiLoading] = useState(false);
  const [canvasAiError, setCanvasAiError] = useState('');
  ...
  const applyLoadedTemplate = (t: any) => {
    setMeta({
    ...
    setRenderMode(t.renderMode === 'canvas' ? 'canvas' : 'document');
  ...
  // subsequent edit autosaves via PUT like an existing one.
  useEffect(() => {
    if (!proposalProjectId) return;

    if (!isNew) {
      apiFetch(`/proposals/templates/${templateId}`).then((res) => {
        if (res.success) applyLoadedTemplate(res.data);
        setLoading(false);
      });
      return;
    }
    ...
    apiFetch('/proposals/templates', {
      method: 'POST',
      body: JSON.stringify({ proposalProjectId, title: EMPTY_META.title, type: EMPTY_META.type, renderMode: 'document' }),
    }).then((res) => {
      if (res.success) {
  ...
  // --- Autosave: persists both contentHtml and pages regardless of mode, so
  // switching between Document and Canvas never loses the other's work. ---
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [meta, branding, renderMode, contentHtml, pages, resolvedId, loading]);

  const setBrandingField = (field: keyof ProposalBranding, value: string) =>
    setBranding((prev) => ({ ...prev, [field]: value }));

  // --- Document mode: live PDF preview (debounced) ---
  const refreshPreview = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl('/proposals/preview-pdf'), {
        ...
        body: JSON.stringify({ title: meta.title, contentHtml, branding, meta }),
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [contentHtml, branding, meta.title]);

  useEffect(() => {
    if (loading || renderMode !== 'document') return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, renderMode, contentHtml, branding, meta.title]);
  ...
  const handleDocGenerate = async () => {
    if (!docAiInstruction.trim()) {
      setDocAiError('Enter a short instruction first.');
      return;
    }
    ...
    try {
      const res = await apiFetch('/proposals/ai-generate', {
        method: 'POST',
        body: JSON.stringify({ instruction: docAiInstruction, projectName, type: meta.type, currency: meta.currency }),
      });
      if (res.success) {
        setContentHtml(res.contentHtml);
        setDocAiProvider(res.provider);
        setContentEditorOpen(false);
        setDocAiInstruction('');
      } else {
        setDocAiError(res.message || 'AI generation failed.');
      }
    } catch (err: any) {
      setDocAiError(err.message || 'Error contacting AI service.');
    } finally {
      setDocAiLoading(false);
    }
  };

  // --- Canvas mode: drag/resize/undo/redo (same mechanics as QuotationEditorPage) ---
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [renderMode, selectedId, editingId, pages]);
  ...
  const handleCanvasGenerate = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setCanvasAiInstruction('');
      } else {
        setCanvasAiError(res.message || 'AI generation failed.');
      }
    } catch (err: any) {
      setCanvasAiError(err.message || 'Error contacting AI service.');
    } finally {
      setCanvasAiLoading(false);
    }
  };
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, overflow: 'hidden' }}>
        ...
        <div style={{ minWidth: 0, overflow: 'hidden' }}>
          <div style={{ fontWeight: 700, fontSize: 12, color: '#fff', lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {branding.companyName.toUpperCase()}
          </div>
          <div style={{ fontSize: 8, color: '#e2e8f0', lineHeight: 1.3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
  ...
  const FooterBandPreview = () => (
    <div className="pointer-events-none" style={{ width: PAGE_W, fontFamily: "'Segoe UI', Arial, sans-serif" }}>
  ...
  const BrandingFields = () => (
    <div className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Company Name</label>
          <input
        ...
        <div>
          <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Tagline</label>
          <input
      ...
      <div>
        <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Logo URL</label>
        <input
          type="url"
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Gradient From</label>
          <input
            type="color"
        ...
        <div>
          <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Gradient To</label>
          <input
            type="color"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Contact Email</label>
          <input
            type="email"
        ...
        <div>
          <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Contact Phone</label>
          <input
            type="tel"
      ...
      <div>
        <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Website</label>
        <input
      ...
      <div>
        <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Footer Address Line 1</label>
        <input
      ...
      <div>
        <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Footer Address Line 2</label>
        <input
    ...
    <div className="h-full min-h-0 flex flex-col bg-[#F5F6F4]">
        ...
        <div className="flex items-center bg-background rounded-xl p-0.5 ml-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[11px] font-bold flex items-center gap-1.5 ${
              renderMode === 'document' ? 'bg-white shadow-sm text-dark' : 'text-slateText'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[11px] font-bold flex items-center gap-1.5 ${
              renderMode === 'canvas' ? 'bg-white shadow-sm text-dark' : 'text-slateText'
            }`}
          >
            ...
            {canvasAiOpen && (
              <div className="absolute right-0 top-10 w-80 bg-white rounded-2xl shadow-2xl border border-dark/10 p-3 space-y-2 z-50">
                ...
                <div className="flex items-center justify-end gap-2">
                  <button type="button" onClick={() => setCanvasAiOpen(false)} className="px-3 py-1.5 rounded-lg text-dark font-bold text-[11px] hover:bg-dark/5">
                    Cancel
                  </button>
                  <button
                    type="button"
                    onClick={handleCanvasGenerate}
                    disabled={canvasAiLoading || !canvasAiInstruction.trim()}
                  ...
                  >
                    {canvasAiLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Wand2 className="w-3.5 h-3.5" />}
                    {canvasAiLoading ? 'Generating' : 'Generate'}
                  </button>
                </div>
              </div>
            )}
      ...
      {renderMode === 'document' ? (
        <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-2">
          {/* Left: document form */}
          <div className="p-5 sm:p-6 overflow-y-auto space-y-4 border-b lg:border-b-0 lg:border-r border-dark/10 bg-white">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
              ...
              <div>
                <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Currency</label>
                <input
                  value={meta.currency}
              ...
              <div>
                <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Prepared For</label>
                <input
              ...
              <div>
                <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Project Type</label>
                <input
              ...
              <div>
                <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Document Ref</label>
                <input
                  value={meta.docRef}
              ...
              <div>
                <label className="block text-[10px] font-bold text-dark mb-1 uppercase tracking-wider">Validity</label>
                <input
              ...
              <div className="p-3 rounded-2xl bg-lime-50 border border-primary/30 space-y-2">
                ...
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[10px] text-slateText">
                    {contentHtml
                      ? docAiProvider
                        ? `Last generated using ${docAiProvider}. Describe changes to regenerate.`
                  ...
                  <button
                    ...
                    disabled={docAiLoading || !docAiInstruction.trim()}
                  ...
                  >
                    {docAiLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Wand2 className="w-3.5 h-3.5" />}
                    {docAiLoading ? 'Generating' : contentHtml ? 'Regenerate' : 'Generate with AI'}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-background border border-dark/10 overflow-hidden">
              <button type="button" onClick={() => setContentEditorOpen((v) => !v)} className="w-full flex items-center justify-between p-4">
                <div>
                  ...
                  <p className="text-[10px] text-slateText mt-0.5">Select any text below to refine just that part with AI.</p>
                </div>
                {contentEditorOpen ? <ChevronUp className="w-4 h-4 text-dark" /> : <ChevronDown className="w-4 h-4 text-dark" />}
              </button>
              {contentEditorOpen && (
          ...
          <div className="p-5 sm:p-6 flex flex-col min-h-[400px] lg:min-h-0 bg-background/60">
            <div className="flex items-center justify-between mb-2 shrink-0">
              ...
              >
                {previewLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5 rotate-180" />}
                Refresh Preview
              </button>
            </div>
            ...
            <div className="flex-1 min-h-[420px] rounded-xl border border-dark/10 overflow-hidden bg-white relative">
              {previewUrl ? (
                ...
                <div className="w-full h-full flex items-center justify-center text-xs text-slateText font-semibold text-center px-6">
                  {previewLoading ? 'Rendering preview' : 'Generate with AI to see the live PDF preview here.'}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex-1 min-h-0 flex">
            ...
            <div className="relative">
              ...
              {addMenuOpen && (
                <div role="menu" className="absolute left-12 top-0 w-40 bg-white rounded-xl shadow-2xl border border-dark/10 py-1.5 z-50">
                  ...
                  <button role="menuitem" onClick={() => addElement('table')} className="focus-ring w-full px-3 py-2 text-left text-xs font-bold text-dark hover:bg-lime-50 flex items-center gap-2">
          ...
          <div className="w-72 shrink-0 bg-white border-l border-dark/10 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {rightTab === 'details' && (
                <>
                  <p className="text-[10px] font-bold uppercase text-slateText tracking-wide">Document Details</p>
                  <div>
                  ...
                  <div className="pt-2 mt-2 border-t border-dark/10">
                    <p className="text-[10px] font-bold uppercase text-slateText tracking-wide mb-3">Branding &amp; Header/Footer</p>
                    <BrandingFields />
                  </div>
                </>
              )}
      ...
      {/* Bottom page/zoom bar - canvas mode only */}
      {renderMode === 'canvas' && (
